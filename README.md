...
```bash
python main.py encrypt -i plaintext.txt -o ciphertext.enc -k 0xABCD
```
...
```bash
python main.py decrypt -i ciphertext.enc -o decrypted.txt -k 0xABCD
```
...
```bash
python attacker.py -i ciphertext.enc 
```
...
```bash
python attacker.py -i ciphertext.enc --ascii
```
